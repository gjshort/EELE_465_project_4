...
int main(void)
{
    ...
    UCA1IFG &= ~UCRXIFG;
    UCA1IE |= UCRXIE;
    uint8_t uart_rx_buf_idx = 0x00;
    char uart_rx_msg_buf[22] = {0};
    ...
    while(1)
    {
        ...
        {
            uart_rx_irq = false;
            if(UCA1RXBUF == '\r' || UCA1RXBUF == '\n' || UCA1RXBUF == '\0')
            {   // Reset index, parse message
                uart_rx_buf_idx = 0;
                parse_uart_msg(uart_rx_msg_buf, &rtc_time);
                rtc_mode = I2C_WRITE;
                write_to_rtc = true;
            }
            else 
            {   // Store received byte into buffer
                uart_rx_msg_buf[uart_rx_buf_idx] = UCA1RXBUF;
                uart_rx_buf_idx++;
            }
        }
